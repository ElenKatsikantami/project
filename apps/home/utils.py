        ...
        try:
            ...
            if data_frame[v_ref['column']].dtype == "O":
                data_frame[v_ref['column']][~data_frame[v_ref['column']].str.isalnum()] = \
                    ...
                    lambda x: x.split("/")[0]).copy()
            # print(data_frame)
            # df_interpretation = data_frame.copy(
            #     )[["LOCA_ID",v_ref['column'],v_ref['elevation']]]
            df_interpretation = data_frame.copy()[["LOCA_ID",v_ref['column']]]
            df_interpretation = df_interpretation[(df_interpretation[v_ref['column']].str.len() != 0)]
            df_interpretation[v_ref['column']] = df_interpretation[v_ref['column']].str.replace('\W', '', regex=True)
            ...
            loca_ids = df_merg.LOCA_ID.unique().tolist()
            for column in df_merg.columns:
                    df_merg[column] = pd.to_numeric(df_merg[column], errors='ignore')
            # df_merg["Elevation"] = df_merg["LOCA_LOCZ"]-df_merg[v_ref['elevation']]
